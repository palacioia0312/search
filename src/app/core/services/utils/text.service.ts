...
export class TextService {
	private readonly bufferABase64 = (buffer: any) =>
		btoa(String.fromCharCode(...new Uint8Array(buffer)));
	private readonly base64ABuffer = (buffer: any) =>
	...
	private async passwordBasedKeyDerivation(
		password: string,
		sal: any,
		iterations: number,
		length: number,
	...
	async encryptContent(content: string): Promise<string> {
		...
		const sal = window.crypto.getRandomValues(
			new Uint8Array(this.LENGTH_SAL)
		);
		const vectorInitialization = window.crypto.getRandomValues(
			new Uint8Array(this.INITIALIZATION_VECTOR_LENGTH)
		);
		const bufferTextFlat = encoder.encode(content);
		const password = await this.passwordBasedKeyDerivation(
			this.pass,
			sal,
			100000,
			256,
	...
	async decryptContent(content: string): Promise<string> {
		...
		const vectorInitialization = dataEncrypted.slice(
			0 + this.LENGTH_SAL,
			this.LENGTH_SAL + this.INITIALIZATION_VECTOR_LENGTH
		);
		const password = await this.passwordBasedKeyDerivation(
			this.pass,
			sal,
